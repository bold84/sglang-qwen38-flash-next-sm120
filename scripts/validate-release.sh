...
[[ "$product" == "sglang-qwen38-flash-next-sm120" ]] || { echo "unexpected product: $product" >&2; exit 1; }
...
[[ "$(jq -er '.model.native_context_length' "$lock")" == 262144 ]] || { echo "native context mismatch" >&2; exit 1; }
...
check_pin QWEN38_RELEASE_VERSION "$version"
check_pin QWEN38_RELEASE_CANDIDATE "$candidate"
check_pin QWEN38_CACHE_SCHEMA "$cache_schema"
check_pin QWEN38_MODEL_REVISION "$(jq -er '.model.revision' "$lock")"
check_pin QWEN38_FLASHINFER_MAIN_HEAD "$(jq -er '.integration.flashinfer.head' "$lock")"
check_pin QWEN38_FLASHINFER_MAIN_TREE "$(jq -er '.integration.flashinfer.tree' "$lock")"
check_pin QWEN38_FLASHINFER_VERSION "$(jq -er '.integration.flashinfer.package_version' "$lock")"
...
grep -Fx "IMAGE=\${IMAGE:-${local_image}}" "$launcher" >/dev/null || {
  echo "launcher does not default to the public bundle tag" >&2; exit 1;
}
grep -F "/srv/cache/sglang-qwen38-flash-next-sm120-${cache_schema}" "$repo/RUN.md" >/dev/null || {
